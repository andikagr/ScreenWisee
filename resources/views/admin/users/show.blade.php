...
@section('page-title', 'Detail: ' . $user->name)
@section('content')

<div class="stats-grid" style="grid-template-columns:repeat(auto-fit,minmax(200px,1fr))">
    <div class="stat-card fade-up">
        <div class="stat-icon blue float-anim"><i data-lucide="user"></i></div>
        <div class="stat-value" style="font-size:18px">{{ $user->name }}</div>
        <div class="stat-label">Kelas: {{ $user->kelas ?? '-' }}</div>
    </div>
    <div class="stat-card fade-up" style="animation-delay:0.1s">
        <div class="stat-icon green bounce-anim"><i data-lucide="bar-chart-2"></i></div>
    ...
    <div class="stat-card fade-up" style="animation-delay:0.3s">
        <div class="stat-icon {{ $posttest ? 'green' : 'yellow' }} pulse-ring"><i data-lucide="check-circle"></i></div>
...
<div class="dashboard-grid-2">
    <div class="card fade-up">
        <div class="card-header"><h3 class="text-primary" style="display:flex;align-items:center;gap:8px;"><i data-lucide="trending-up"></i> Grafik Waktu Main HP</h3></div>
    ...
    <div class="card fade-up" style="animation-delay:0.1s">
        <div class="card-header"><h3 class="text-accent" style="display:flex;align-items:center;gap:8px;"><i data-lucide="pie-chart"></i> Distribusi Aktivitas</h3></div>
...
<div class="card fade-up" style="border:4px solid var(--accent-yellow);border-radius:32px">
    <div class="card-header" style="background:var(--accent-yellow);border-bottom:none; display: flex; justify-content: space-between; align-items: center;">
        <h3 style="color:var(--dark-900);font-size:20px; margin: 0; display:flex; align-items:center; gap:8px;"><i data-lucide="clipboard-list"></i> Riwayat Jurnal Siswa</h3>
        <a href="{{ route('export.student.pdf', $user->id) }}" class="btn btn-sm pulse-ring" style="background: var(--accent-pink-dark); color: white; border: none; box-shadow: 0 4px 0 #be185d;">📄 Export PDF</a>
    </div>
    <div class="card-body">
            ...
            <table style="width:100%">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>@if($t->screenshot_url)<a href="{{ $t->screenshot_url }}" target="_blank" class="btn btn-sm bounce-anim" style="background:var(--accent-pink-dark);color:white;border:none">📷 Lihat</a>@else - @endif</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
...
<div style="margin-top:24px"><a href="{{ route('admin.users.index') }}" class="btn btn-lg" style="background:var(--dark-100);color:var(--dark-800);box-shadow:0 8px 0 var(--dark-200)">← Kembali ke Kelola User</a></div>
@endsection
