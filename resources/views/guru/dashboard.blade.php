...
@section('content')
@php $unassignedCount = $siswaStats->filter(fn($s) => is_null($s->user->guru_id))->count(); @endphp

<div class="stats-grid" style="grid-template-columns:repeat(auto-fit, minmax(200px, 1fr))">
    <div class="stat-card fade-up"><div class="stat-icon blue float-anim"><i data-lucide="users"></i></div><div class="stat-value">{{ $totalSiswa }}</div><div class="stat-label">Total Murid</div></div>
    <div class="stat-card fade-up" style="animation-delay:0.1s"><div class="stat-icon green bounce-anim"><i data-lucide="check-circle-2"></i></div><div class="stat-value">{{ $siswaTrackingToday }}</div><div class="stat-label">Udah Ngisi Hari Ini</div></div>
    ...
    <div class="stat-card fade-up" style="animation-delay:0.3s"><div class="stat-icon pink pulse-ring"><i data-lucide="alert-triangle"></i></div><div class="stat-value">{{ $totalSiswa - $siswaTrackingToday }}</div><div class="stat-label">Belum Ngisi Jurnal</div></div>
</div>

@if($unassignedCount > 0)
<div class="alert alert-info" style="margin-bottom:24px">
    <i data-lucide="info" style="width:20px; height:20px; display:inline-block; margin-right:8px; vertical-align:-5px;"></i> Ada <strong>{{ $unassignedCount }} siswa</strong> yang belum di-assign ke guru manapun — mereka ditampilkan di sini agar bisa dimonitor. Minta Admin untuk assign siswa ke guru melalui menu <strong>Kelola User → Edit</strong>.
</div>
@endif
...
<div class="dashboard-grid-2">
    ...
    <div class="card fade-up" style="animation-delay:0.2s">
        <div class="card-header"><h3 style="color:var(--accent-pink-dark);font-size:18px;display:flex;align-items:center;gap:8px;"><i data-lucide="bar-chart-2" style="width:20px;height:20px;"></i> Rata-rata Per Kelas</h3></div>
...
<div class="card fade-up" style="margin-bottom:32px; border:4px solid var(--accent-yellow); border-radius:32px">
    <div class="card-header" style="background:var(--accent-yellow);border-bottom:none"><h3 style="color:var(--dark-900);font-size:20px;display:flex;align-items:center;gap:8px;"><i data-lucide="camera" style="width:24px;height:24px;"></i> Laporan Screenshot Terbaru</h3></div>
    <div class="card-body">
...
<div class="card fade-up" style="margin-bottom:32px; border:3px solid var(--accent-purple); border-radius:24px">
    <div class="card-header" style="background:var(--primary-50); border-bottom:none">
        <h3 style="color:var(--primary-700); font-size:18px; display:flex; align-items:center; gap:8px;">
            <i data-lucide="trophy" style="width:20px;height:20px;color:var(--accent-purple-dark)"></i> Top 5 Screen Time Terendah (Siswa Bimbinganmu)
        </h3>
    </div>
...
<div class="card fade-up">
    <div class="card-header"><h3 style="display:flex;align-items:center;gap:8px;"><i data-lucide="users" style="width:20px;height:20px;"></i> Daftar Siswa</h3><button class="btn btn-sm btn-outline" onclick="exportCSV()"><i data-lucide="download" style="width:16px;height:16px;"></i> Export CSV</button></div>
    <div class="card-body">
        @if($siswaStats->count())
            ...
            <table id="siswaTable">
                ...
                <tbody>
                    ...
                    <tr data-name="{{ strtolower($s->user->name) }}" data-kelas="{{ $s->user->kelas ?? '' }}" data-status="{{ $s->todayTracking ? 'tracked' : 'untracked' }}">
                        ...
                        <td>{!! $s->todayTracking ? '<i data-lucide="check-circle-2" style="color:var(--success); width:20px; height:20px;" class="bounce-anim"></i>' : '<i data-lucide="x-circle" style="color:var(--danger); width:20px; height:20px; opacity:0.5;"></i>' !!}</td>
                        ...
                        <td>
                            {!! $s->hasPretest ? '<span class="badge badge-green" style="margin-right:4px">Pre <i data-lucide="check" style="width:12px;height:12px;margin-left:2px;"></i></span>' : '<span class="badge badge-gray" style="margin-right:4px">Pre <i data-lucide="x" style="width:12px;height:12px;margin-left:2px;"></i></span>' !!}
                            {!! $s->hasPosttest ? '<span class="badge badge-green">Post <i data-lucide="check" style="width:12px;height:12px;margin-left:2px;"></i></span>' : '<span class="badge badge-gray">Post <i data-lucide="x" style="width:12px;height:12px;margin-left:2px;"></i></span>' !!}
                        </td>
        ...
        <div id="noResults" style="display:none;text-align:center;padding:40px 20px;color:var(--dark-400);font-weight:700;font-size:16px"><i data-lucide="frown" style="width:48px;height:48px;margin:0 auto 12px;display:block;"></i> Yah, nggak nemu nama siswanya nih</div>
        @else
        <div class="empty-state fade-up"><div class="empty-state-icon float-anim"><i data-lucide="users" style="width:64px;height:64px;margin:0 auto;"></i></div><h3 style="color:var(--primary-600)">Belum Ada Murid Nih</h3><p style="color:var(--dark-500);font-weight:600">Santai aja, murid-murid bakal muncul kalau udah di-assign sama Admin ya!</p></div>
        @endif
    </div>
</div>
@endsection
...
function exportCSV() {
    ...
    rows.forEach(r => {
        ...
        csv += [cells[0].textContent.trim(), cells[1].textContent.trim(), cells[2].textContent.trim(),
                cells[3].textContent.trim(), cells[4].querySelector('i') ? 'Ya' : 'Tidak',
