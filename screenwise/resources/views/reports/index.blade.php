...
<div class="stats-grid" style="grid-template-columns:1fr 1fr 1fr 1fr">
    <div class="stat-card"><div class="stat-icon blue">👥</div><div class="stat-value">{{ $totalSiswa }}</div><div class="stat-label">Total Siswa</div></div>
    <div class="stat-card"><div class="stat-icon green">✅</div><div class="stat-value">{{ $completedBoth }}</div><div class="stat-label">Lengkap Pre+Post</div></div>
    <div class="stat-card"><div class="stat-icon yellow">📝</div><div class="stat-value">{{ round($avgPreScreenTime,1) }} jam</div><div class="stat-label">Avg Pre-Test</div></div>
    <div class="stat-card"><div class="stat-icon green">✅</div><div class="stat-value">{{ round($avgPostScreenTime,1) }} jam</div><div class="stat-label">Avg Post-Test</div></div>
</div>
...
<div class="challenge-card" style="margin-bottom:28px;{{ $change <= 0 ? '' : 'background:linear-gradient(135deg,#ef4444,#dc2626)' }}">
    <h3>{{ $change <= 0 ? '🎉 Program Berhasil!' : '⚠️ Perlu Evaluasi' }}</h3>
                    ...
                    <tr>
                        ...
                        <td>
                            <div style="display:flex;align-items:center;justify-content:space-between;">
                                ...
                                <a href="{{ route('admin.users.show', $r->siswa) }}" class="btn btn-sm" style="background:var(--accent-yellow);color:var(--dark-800);border:none;padding:4px 8px;font-size:12px;">👁️ Detail</a>
                            </div>
                        </td>
                    </tr>
                    @endforeach
