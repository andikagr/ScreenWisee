...
<div class="card fade-up" style="border:4px solid var(--accent-pink);border-radius:32px;overflow:hidden">
    <div class="card-header" style="background:var(--accent-pink);border-bottom:none;padding:24px">
        <h3 style="color:var(--dark-900);font-size:24px;text-align:center;width:100%">📝 Misi Awal: Ceritain Kebiasaanmu!</h3>
    </div>
    <div class="card-body" style="padding:32px">
        ...
        <form method="POST" action="{{ route('siswa.pretest.store') }}" id="pretestForm" novalidate>
            ...
            <div class="form-group" style="margin-bottom:24px">
                <label class="form-label" style="font-size:16px;color:var(--dark-800)">Sehari biasanya main HP berapa jam? <span style="color:var(--danger)">*</span></label>
                <input type="number" name="avg_screen_time" id="avgScreenTime" class="form-input" step="0.5" min="0" max="24" value="{{ old('avg_screen_time', $pretest?->avg_screen_time) }}" required placeholder="Misalnya: 5" style="border-radius:20px;font-size:18px;padding:16px;text-align:center">
            ...
            <div class="form-row" style="gap:24px;margin-bottom:24px">
                <div class="form-group" style="flex:1">
                    <label class="form-label" style="font-size:16px;color:var(--dark-800)">😴 Biasanya tidur jam berapa? <span style="color:var(--danger)">*</span></label>
                ...
                <div class="form-group" style="flex:1">
                    <label class="form-label" style="font-size:16px;color:var(--dark-800)">🌅 Biasanya bangun jam berapa? <span style="color:var(--danger)">*</span></label>
            ...
            <div class="form-group" style="margin-bottom:32px">
                <label class="form-label" style="font-size:16px;color:var(--dark-800)">Ngapain aja sih kalau main HP? (Boleh pilih lebih dari satu)</label>
                <div style="display:grid;grid-template-columns:1fr;gap:12px">
                    ...
                    <label class="form-check" style="background:var(--primary-50);border:3px solid var(--primary-100);padding:16px;border-radius:20px;cursor:pointer">
                        <input type="checkbox" name="gadget_habits[]" value="{{ $h }}" {{ in_array($h, old('gadget_habits', $pretest?->gadget_habits ?? [])) ? 'checked' : '' }} style="width:24px;height:24px">
            ...
            <div class="form-group" style="margin-bottom:32px">
                <label class="form-label" style="font-size:16px;color:var(--dark-800)">Ada curhatan lain soal kebiasaan digitalmu?</label>
                <textarea name="notes" class="form-textarea" maxlength="1000" placeholder="Contoh: Aku sering susah tidur gara-gara kepikiran game..." style="border-radius:20px;padding:16px;font-size:15px;min-height:120px">{{ old('notes', $pretest?->notes) }}</textarea>
                <div class="form-hint" id="notesCount" style="text-align:right">0/1000 huruf</div>
            </div>
            <button type="submit" class="btn btn-lg" id="submitBtn" style="background:var(--accent-pink-dark);color:white;width:100%;justify-content:center;box-shadow:0 8px 0 #be185d;padding:16px;font-size:20px">🚀 Simpan Ceritaku!</button>
        </form>
    </div>
</div>
@endsection
